...
export default class TabBar extends React.PureComponent<IProps, any> {
  ...
  renderTab = (section: SectionListData<any>, key: number) => {
    const {onPress, currentIndex} = this.props;
    ...
    return (
      <Ripple
        ...
        key={key}
        borderBottomWidth={2}
        borderBottomColor={isActive ? 'red.600' : 'muted.100'}
        onLayout={this.onTabLayout(key)}>
        <Heading
          size="sm"
          mx={4}
          my={3}
          color={isActive ? 'red.600' : 'muted.500'}>
          {section.title}
        </Heading>
      </Ripple>
    );
    ...
    return (
      <View style={[{width: WindowWidth}, tabBarStyle]}>
        <ScrollView
          ...
          <HStack onLayout={this.onTabContainerLayout}>
